...
function goToPrediction() {
    document.getElementById('prediction-greeting').textContent = `Before we start, ${username} — what % do you think you use AI at?`;
...
function submitPrediction() {
    const prediction = parseInt(document.getElementById('prediction-slider').value);
...
function goToCalculating() {
    document.getElementById('calculating-text').textContent = `Analysing ${username}'s AI behaviour patterns...`;
...
const categoryMessages = {
    context: { low: "You're sending AI into tasks with almost no briefing.", mid: "You give AI context, but constraints and format are missing.", high: "Strong briefing habit." },
    iteration: { low: "First output = final output.", mid: "You iterate when something's wrong.", high: "You diagnose failures and fix prompts." },
    decomp: { low: "Complex tasks go in as one big prompt.", mid: "You split tasks sometimes.", high: "You design task sequences." },
    evaluation: { low: "If it sounds right, you use it.", mid: "You check for obvious errors.", high: "You evaluate before you use." },
    workflow: { low: "Every AI session starts from scratch.", mid: "You have some saved prompts.", high: "You maintain documented workflows." }
};
...
function getGapMessage(prediction, actual) {
    ...
    if (direction === 'higher') {
        if (gap <= 15) return `You overestimated by ${gap} points. That's what we're here to close.`;
        if (gap <= 30) return `You predicted ${prediction}%. You scored ${actual}%. That ${gap}-point gap is valuable.`;
...
function showScoreReveal() {
    ...
    setTimeout(() => {
        document.getElementById('prediction-display').textContent = `You predicted: ${prediction}%`;
    }, 800);
    ...
    setTimeout(() => {
        ...
        document.getElementById('archetype-subtitle').textContent = `Score: ${result.total}%`;
    }, 3300);
...
function showArchetypePage() {
    ...
    document.getElementById('archetype-headline').textContent = archetype.headline.replace('[NAME]', name);
    document.getElementById('archetype-whats-happening').textContent = archetype.whatsHappening;
...
function goToChallenge() {
    ...
    document.querySelector('.challenge-badge').textContent = `Day ${currentDay} Challenge`;
    ...
    if (challengesDone.includes(currentDay)) {
        ...
        completeBtn.onclick = function() { completeAndAdvance(currentDay); };
    }
...
function retakeQuiz() {
    ...
    if (quizDate) {
        const daysSince = Math.floor((new Date() - new Date(quizDate)) / (1000 * 60 * 60 * 24));
        if (daysSince < 7) {
